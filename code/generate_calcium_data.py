...
def add_calcium_drives(net, seed=0):
...
def scale_calcium(net, scale_factor=1.0):
    """Multiple calcium conductances by scale factor"""
    for sec_name in net.cell_types['L5_pyramidal'].sections.keys():
        print(f'Updating: {sec_name}')
        gbar_ca = net.cell_types['L5_pyramidal'].sections[sec_name].mechs['ca']['gbar_ca']
        gbar_ca = np.array(gbar_ca) * scale_factor
        net.cell_types['L5_pyramidal'].sections[sec_name].mechs['ca']['gbar_ca'] = gbar_ca.tolist()

def run_calcium_simulation(net_base, calcium_scale, tstop, seed):
    net = net_base.copy()
...
calcium_list = [1.0, 2.0, 5.0, 10.0]
calcium_sweep = np.repeat(calcium_list, n_trials)
...
res = Parallel(n_jobs=n_trials)(
    delayed(run_calcium_simulation)(net_base, scale, tstop, seed_idx) for seed_idx, scale in enumerate(calcium_sweep))

dpl_dict = {
    'trial': np.tile(np.arange(n_trials), (len(calcium_list))),
    'calcium_scale': calcium_sweep,
...
df.to_pickle(f'{data_path}/calcium_simulations.pkl')